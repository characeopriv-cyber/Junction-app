...
function mapInventoryRow(raw) {
  ...
  return {
    ...
    floor: pick("floor"), status: pick("status") || "available", raw,
  };
}
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/properties?action=inventory", {
        ...
        body: JSON.stringify({ ...meta, units: parsedUnits }),
      });
    ...
    <div className="fixed inset-0 z-[70] overflow-y-auto" style={{ background: "#fff" }}>
      ...
      {loading ? (
        ...
        <div className="p-4">
          ...
          {data.inventory.description && <p className="text-xs mb-4" style={{ color: T.sub }}>{data.inventory.description}</p>}
...
function MessagesView({ currentUser, onSignIn }) {
  ...
  const startNewChat = async () => {
    ...
    try {
      const lookup = await fetch(`/api/conversations?action=lookup&email=${encodeURIComponent(newChatEmail.trim())}`, { credentials: "include" }).then((r) => r.json());
