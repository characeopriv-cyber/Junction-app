...
function CommunityView({ onOpenPost, currentUserId }) {
  ...
  // Pull real member counts from the database once loaded, so the numbers
  // shown are real once anyone has actually joined — falls back silently
  // to the seeded mock counts if the backend isn't connected yet.
  useEffect(() => {
    fetch("/api/circles")
      ...
      .then((data) => {
        ...
        setCircles((prev) =>
          prev.map((c) => {
            const real = data.circles.find((r) => r.code === c.code);
            return real && Number(real.total) > 0 ? { ...c, total: Number(real.total) } : c;
          })
        );
      })
      .catch(() => {});
  ...
  if (circle) {
    const posts = CIRCLE_POSTS.filter((p) => p.circle === circle.code);
    ...
    return (
      <div className="pb-6">
        ...
        <div className="flex gap-2 px-4 pt-3">
          <button className="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold py-2 rounded-lg"
        ...
        <div className="px-4 pt-4">
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={onOpenPost} className="text-xs font-semibold" style={{ color: T.signal }}>+ Post</button>
          </div>
...
function AppInner() {
  ...
  // of the mock set. If the backend isn't connected yet (no DATABASE_URL),
  // this silently no-ops and the app keeps working on mock data alone.
  useEffect(() => {
    fetch("/api/properties")
      ...
      .catch(() => {}); // backend not connected yet — fine, mock data covers it
  }, []);
  ...
  const handlePublish = (newProperty) => {
    // Show it immediately for a snappy UI, then persist for real in the background.
    ...
    }).catch(() => {}); // backend not connected yet — the listing still shows locally either way
  };
    ...
    <div style={{ background: T.paper, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      {tab === "reels" ? (
        ...
        <div key={tab} className="tab-fade pb-16 md:pb-0">
          ...
          {tab === "community" && <CommunityView onOpenPost={() => setShowPostModal(true)} currentUserId={currentUser?.name} />}
