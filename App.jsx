  ...
  const send = async () => {
    ...
    try {
      ...
      setThreads((prev) =>
        prev.map((t) =>
          t.id === activeId
            ? { ...t, messages: [...t.messages, { from: "system", text: `Message couldn't be delivered — ${e.message}` }] }
            : t
        )
      );
    } finally {
...
function AuthModal({ onClose, onAuthed }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || `${mode === "login" ? "Sign in" : "Sign up"} failed.`);
        setBusy(false);
        return;
      }
      onAuthed(data.user);
    ...
    <div style={{ background: T.paper, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      {tab === "reels" ? (
        ...
        <div key={tab} className="tab-fade pb-16 md:pb-0">
          ...
          {tab === "community" && <CommunityView onOpenPost={() => setShowPostModal(true)} currentUserId={currentUser?.id} />}
