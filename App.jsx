...
function MessagesView({ currentUser, onSignIn }) {
  ...
  const otherUserId = activeThread
    ? String(activeThread.user_a_id) === String(currentUser?.id) ? activeThread.user_b_id : activeThread.user_a_id
    : null;

  // Load real threads for the signed-in user.
  useEffect(() => {
  ...
  // Load messages for the active human thread, poll while it's open.
  useEffect(() => {
  ...
  useEffect(() => {
    const ids = threads.map((t) => (String(t.user_a_id) === String(currentUser?.id) ? t.user_b_id : t.user_a_id));
    if (ids.length === 0) return;
    const poll = () => fetch(`/api/presence?userIds=${ids.join(",")}`)
      .then((r) => (r.ok ? r.json() : null))
  ...
  const sendMessage = async ({ type = "text", text, mediaUrl, mediaMeta } = {}) => {
    ...
    const payload = { threadId: activeId, senderId: currentUser.id, type, text: text ?? undefined, mediaUrl, mediaMeta };

    // Optimistic local render.
    const optimistic = { id: `local-${Date.now()}`, thread_id: activeId, sender_id: currentUser.id, type, body: text, media_url: mediaUrl, media_meta: mediaMeta, created_at: new Date().toISOString() };
    setThreadMessages((p) => [...p, optimistic]);

    if (!isOnline) {
      setOutbox((p) => [...p, { payload, queuedAt: Date.now() }]);
      return;
    }
    try {
      const res = await fetch("/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!res.ok) setOutbox((p) => [...p, { payload, queuedAt: Date.now() }]);
    } catch {
      setOutbox((p) => [...p, { payload, queuedAt: Date.now() }]);
    }
  };
  ...
  const startNewChat = async () => {
    ...
    try {
      const lookup = await fetch(`/api/messages?lookupEmail=${encodeURIComponent(newChatEmail.trim())}`).then((r) => r.json());
      if (!lookup.user) { setNewChatError("No Junction user found with that email."); return; }
      if (String(lookup.user.id) === String(currentUser.id)) { setNewChatError("That's you!"); return; }
      const created = await fetch("/api/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: currentUser.id, otherUserId: lookup.user.id, senderId: currentUser.id, type: "text", text: `Hi ${lookup.user.name}! 👋` }),
      }).then((r) => r.json());
      setThreads((p) => [{ id: created.threadId, user_a_id: currentUser.id, user_b_id: lookup.user.id, context_label: null, last_body: `Hi ${lookup.user.name}! 👋` }, ...p]);
      setActiveId(created.threadId);
  ...
  return (
    <div className="flex h-full" style={{ minHeight: "70vh" }}>
      <div className={`${mobileView === "chat" ? "hidden" : "flex"} sm:flex w-full sm:w-72 border-r flex-col`} style={{ borderColor: T.line }}>
        ...
        {showNewChat && (
          <div className="p-3 border-b" style={{ borderColor: T.line, background: T.paper }}>
            <div className="text-[11px] font-semibold mb-1.5" style={{ color: T.sub }}>Start a chat by email</div>
            <input value={newChatEmail} onChange={(e) => setNewChatEmail(e.target.value)}
              placeholder="their@email.com" className="w-full text-sm px-2.5 py-1.5 rounded-lg border outline-none mb-1.5" style={{ borderColor: T.line }} />
            {newChatError && <div className="text-[11px] mb-1.5" style={{ color: "#E0554C" }}>{newChatError}</div>}
            <div className="flex gap-2">
              <button onClick={startNewChat} className="flex-1 text-xs font-semibold py-1.5 rounded-lg" style={{ background: T.ink, color: "#fff" }}>Start</button>
              <button onClick={() => setShowNewChat(false)} className="text-xs font-semibold px-3 py-1.5 rounded-lg" style={{ background: T.panel, color: T.sub }}>Cancel</button>
            </div>
          </div>
        )}
        ...
        <div className="overflow-y-auto flex-1">
          ...
          {threads.map((t) => {
            const otherId = String(t.user_a_id) === String(currentUser.id) ? t.user_b_id : t.user_a_id;
            const status = presence[otherId] || "offline";
            return (
              <button key={t.id} onClick={() => { setActiveId(t.id); setMobileView("chat"); }}
                className="w-full text-left p-3 border-b flex items-center gap-3"
...
function JobsView({ verifyStatuses, currentUser, onSignIn, services }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/candidate-profile?userId=${currentUser.id}`)
      .then((r) => (r.ok ? r.json() : null))
  ...
  const rsvp = async (event) => {
    ...
    try {
      const res = await fetch("/api/events", {
        ...
        body: JSON.stringify({ action: "rsvp", eventId: event.id, userId: currentUser.id }),
      });
      const data = await res.json();
...
function PassportView({ currentUser, properties, services, statuses, setStatuses, settings, setSettings, onSignOut, onSignIn, onUserUpdated }) {
  ...
  const patchUser = async (fields) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => null);
      if (!res.ok) {
        setSaveMsg(`Didn't save — ${data?.error || res.status}. Your Passport backend may need the users table columns added (see /api/users.js comments).`);
        return null;
      }
      const u = data.user || {};
      onUserUpdated?.({
