...
function FeedView({ liveViews, properties }) {
  ...
  const handleSelect = (id) => {
    setActiveOrb((prev) => {
      if (prev === id) {
        // Tapping the already-active orb again returns to "For You" and
        // resumes the rolling animation.
        setPaused(false);
        return "forYou";
      }
      setPaused(true);
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end sm:items-center justify-center" style={{ background: "rgba(20,25,31,0.5)" }}>
      <div
        ...
        style={{ background: "#fff", maxHeight: "min(88vh, 640px)", minHeight: 0 }}
      >
        ...
        {!emiratesIdOk ? (
          ...
          <div className="p-4 border-t sticky bottom-0" style={{ borderColor: T.line, background: "#fff" }}>
            <button
              onClick={() => {
                onPublish({
                  id: `s${Date.now()}`,
