...
function MessagesView({ currentUser, onSignIn }) {
  ...
  useEffect(() => {
    ...
    const poll = () => fetch(`/api/conversations?action=presence&userIds=${ids.join(",")}`)
      .then((r) => (r.ok ? r.json() : null))
  ...
  const sendMessage = async ({ type = "text", text, mediaUrl, mediaMeta } = {}) => {
    ...
    try {
      const res = await fetch(`/api/conversations/${activeId}/messages`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  const uploadAndSend = async (file, kind) => {
    ...
    try {
      const res = await fetch("/api/people?action=upload", { method: "POST", body: form });
  ...
  const startNewChat = async () => {
    ...
    try {
      const lookup = await fetch(`/api/conversations?action=lookup&email=${encodeURIComponent(newChatEmail.trim())}`).then((r) => r.json());
      ...
      const created = await fetch("/api/conversations", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ participantIds: [currentUser.id, lookup.user.id] }),
      }).then((r) => r.json());
      const conversationId = created.conversation.id;
      await fetch(`/api/conversations/${conversationId}/messages`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ senderId: currentUser.id, body: `Hi ${lookup.user.name}! 👋` }),
      });
      setThreads((p) => [{ id: conversationId, participant_ids: [currentUser.id, lookup.user.id], context_label: null, last_body: `Hi ${lookup.user.name}! 👋` }, ...p]);
...
function JobsView({ verifyStatuses, currentUser, onSignIn, services }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/people?action=candidate&userId=${currentUser.id}`)
      .then((r) => (r.ok ? r.json() : null))
