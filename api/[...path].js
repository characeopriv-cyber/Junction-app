      ...
      if (sub === "login" && method === "POST") {
        ...
        if (!profile) {
          const { data: created } = await authed
            .from("profiles")
            .insert({ id: data.user.id, email, name: email.split("@")[0], junction_id: junctionIdFor(data.user.id), passport_tier: "ordinary" })
            .select()
            .maybeSingle();
          profile = created;
        }
    ...
    if (resource === "circles") {
      ...
      if (code && segments[2] === "posts") {
        ...
        if (method === "POST") {
          ...
          const { data, error } = await sb
            .from("circle_posts")
            .insert({ circle_id: circle.id, title: body.title, type: body.type || "announcement", author_id: user?.id || body.authorId || null })
            .select()
            .maybeSingle();
          if (error) return sendJson(res, 400, { error: error.message });
      ...
      if (method === "POST") {
        ...
        const { data, error } = await sb
          .from("circles")
          .insert({ code, name: body.name, flag: body.flag || null, created_by: user?.id || null })
          .select()
          .maybeSingle();
        if (error) return sendJson(res, 400, { error: error.message });
        if (user) await sb.from("circle_members").insert({ circle_id: data.id, user_id: user.id }).catch(() => {});
