  ...
  try {
    ...
    if (resource === "auth") {
        ...
        if (!profile) {
          ...
          const { data: created } = await authed
            ...
            .insert({
              ...
              name: email.startsWith("phone_") ? "Junction Member" : email.split("@")[0],
      ...
      if (sub === "register" && method === "POST") {
        const body = await readBody(req);
        let { email, password, name, country, age, accountType, companyName, phone } = body || {};
    ...
    if (resource === "properties") {
      ...
      if (method === "POST" && action === "inventory-ai-parse") {
        ...
        if (mimetype === "application/pdf") {
          ...
          return sendJson(res, 400, {
            error: "Junction AI can read PDFs, Excel, Word docs, and photos/scans of a rent roll or sale sheet.",
          });
        }
