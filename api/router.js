...
export default async function handler(req, res) {
  try {
    ...
    if (resource === "properties") {
      ...
      if (method === "POST") {
        ...
        const { data, error } = await sb
          ...
          .maybeSingle();
        if (error) return sendJson(res, 400, { error: error.message });
        return sendJson(res, 200, { property: { ...data, type: data.listing_type || "Sale", priceFreq: data.listing_type === "Rent" ? "yr" : undefined } });
      }
    ...
    if (resource === "music" && method === "GET") {
      const { data, error } = await sb.from("music_tracks").select("*").order("genre");
      if (error) return sendJson(res, 400, { error: error.message });
