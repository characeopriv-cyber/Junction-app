...
export default async function handler(req, res) {
  try {
    ...
    // confirmed fixed. Exposes no row data, just connectivity facts, so
    // it's safe to leave briefly, but shouldn't stay in the codebase.
    if (resource === "diag" && req.query.action === "svc-check") {
      const out = { hasEnvVar: !!(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE) };
      try {
        const svc = adminClient();
        const { count, error } = await svc.from("admin_users").select("id", { count: "exact", head: true });
        out.queryOk = !error;
        out.rowCountVisible = count ?? null;
        out.errorMessage = error?.message || null;
