...
function FeedView({ liveViews, properties, currentUser, onPropertyUpdated, onRequireSignIn }) {
            ...
            <label className="border-2 border-dashed rounded-lg p-4 text-center text-xs block cursor-pointer" style={{ borderColor: T.line, color: T.sub }}>
              ...
              {photoUploading ? (
                ...
                <div className="flex flex-col items-center gap-1">
                  <img src={photoUrl} alt="" className="h-20 rounded-lg object-cover" />
                  <span style={{ color: T.signal }}>Photo attached — tap to change</span>
                </div>
              ) : (
                <><Upload size={16} className="mx-auto mb-1" /> Tap to add a photo</>
              )}
...
const COUNTRIES = [
  "United Arab Emirates","Pakistan","India","Philippines","Egypt","United Kingdom","United States",
  "Saudi Arabia","Jordan","Lebanon","Syria","Nigeria","Bangladesh","Sri Lanka","Nepal","China",
  "Russia","France","Germany","Canada","Australia","South Africa","Kenya","Morocco","Tunisia",
  "Turkey","Iran","Iraq","Yemen","Sudan","Somalia","Afghanistan","Indonesia","Malaysia","Other",
...
function AuthModal({ onClose, onAuthed }) {
  ...
  const submit = async () => {
    setError("");
    if (!email.trim() || !password.trim() || (mode === "register" && (!name.trim() || !country))) {
      setError(mode === "register" && !country ? "Select your country to continue." : "Fill in all fields.");
      return;
    }
    setBusy(true);
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(
          mode === "login" ? { email, password } : { email, password, name, country }
        ),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) {
