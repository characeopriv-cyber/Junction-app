...
function PropertyDetailModal({ p, currentUser, onClose, onChat, onEdit, onDeleted, likedIds, onToggleLike, ownerProfile, onRequireSignIn }) {
  ...
  const facts = [
    p.beds != null && { icon: BedDouble, label: `${p.beds} Bed${p.beds === 1 ? "" : "s"}` },
    p.baths != null && { icon: Bath, label: `${p.baths} Bath${p.baths === 1 ? "" : "s"}` },
    p.sqft != null && { icon: Maximize, label: `${p.sqft.toLocaleString()} sqft` },
    ...
    <div className="fixed inset-0 z-50" style={{ background: "#04060A" }}>
      ...
      <div className="relative w-full transition-all duration-300" style={{ height: mounted ? "56vh" : "40vh", opacity: mounted ? 1 : 0.4 }}>
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
          ...
          <button onClick={() => onToggleLike(p.id)} className="w-11 h-11 rounded-full flex items-center justify-center shrink-0" style={{ background: "rgba(0,0,0,.4)", backdropFilter: "blur(4px)" }}>
            <Heart size={19} style={{ color: liked ? "#E0554C" : "#fff", fill: liked ? "#E0554C" : "none" }} />
          </button>
        </div>
      </div>
      ...
      <div className="p-3.5 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t" style={{ borderColor: T.line }}>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Heart size={16} style={{ color: liked ? "#E0554C" : T.sub, fill: liked ? "#E0554C" : "none" }} />
            </button>
            <button
...
function ReelCard({ p, views, liked, onLike, onChat, onCall, onShare, isFirst }) {
  return (
    <div
    ...
    >
      {p.video_url && (
        <video src={p.video_url} className="absolute inset-0 w-full h-full object-cover" autoPlay loop muted playsInline />
      )}
      {(p.photo_url || p.video_url) && (
...
function ServiceReelCard({ s, liked, onLike, onChat, onCall, onShare, isFirst }) {
  return (
    <div
      className="relative w-full h-full flex flex-col justify-end overflow-hidden"
    ...
    fetch("/api/jobs")
      ...
      .then((data) => {
        ...
        const real = data.jobs.map((j) => ({
          ...
          jobType: j.job_type, salaryRange: j.salary_range, location: j.location,
          description: j.description, ownerId: j.owner_id,
        }));
    ...
    <div className="rounded-2xl overflow-hidden border mb-3" style={{ borderColor: T.line, background: "#fff" }}>
      <div className="h-20 relative flex items-center justify-between px-4" style={{ background: `linear-gradient(135deg,${cat.color},#0A0F1A)` }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onToggleLike(event.id)} className="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
            style={{ background: "rgba(0,0,0,.35)", backdropFilter: "blur(4px)" }}>
            <Heart size={14} fill={liked ? "#E0554C" : "none"} color={liked ? "#E0554C" : "#fff"} />
          </button>
        </div>
      </div>
      <div className="p-3.5">
        ...
        <div className="flex items-center justify-between mt-2.5">
          <div className="text-xs font-semibold flex items-center gap-2" style={{ color: T.ink }}>
            ...
            <span className="flex items-center gap-0.5" style={{ color: T.sub, fontWeight: 500 }}><Eye size={11} /> {event.views || 0}</span>
          </div>
...
function EventsView({ currentUser, onSignIn }) {
  ...
  const toggleLike = async (eventId) => {
    ...
    try {
      ...
      if (!res.ok) setLikedIds((prev) => (prev.includes(eventId) ? prev.filter((id) => id !== eventId) : [...prev, eventId]));
    } catch {
      setLikedIds((prev) => (prev.includes(eventId) ? prev.filter((id) => id !== eventId) : [...prev, eventId]));
    }
  };
