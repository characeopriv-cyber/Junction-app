        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select value={form.category} onChange={(e) => update("category", e.target.value)}
              className="text-sm px-3 py-2 rounded-lg border outline-none" style={{ borderColor: T.line }}>
              {["Apartment","Villa","Townhouse","Penthouse","Land","Office","Retail","Warehouse","Building"].map((c) => (
...
function PropertyDetailModal({ p, currentUser, onClose, onChat, onEdit, onDeleted, likedIds, onToggleLike, ownerProfile }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,.55)" }}
      onClick={onClose}>
      <div className="w-full sm:w-[480px] sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto" style={{ background: "#fff" }}
        onClick={(e) => e.stopPropagation()}>
        <div className="h-72 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${p.grad?.[0] || "#0E2A44"}, ${p.grad?.[1] || "#163A5C"})` }}>
          {p.video_url ? (
              ...
              {photos.length > 1 && (
                <>
                  <button onClick={() => setPhotoIdx((i) => (i - 1 + photos.length) % photos.length)}
                    className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(0,0,0,.4)" }}>
                    <ChevronRight size={16} color="#fff" style={{ transform: "rotate(180deg)" }} />
                  </button>
                  <button onClick={() => setPhotoIdx((i) => (i + 1) % photos.length)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(0,0,0,.4)" }}>
                  ...
                  <div className="absolute top-3 left-3 text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "rgba(0,0,0,.5)", color: "#fff" }}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5" style={{ color: "rgba(255,255,255,.6)" }}>
              <Camera size={22} />
              <span className="text-xs">No photos yet</span>
            </div>
          )}
          <button onClick={onClose} className="absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(0,0,0,.5)" }}>
            <X size={16} color="#fff" />
          </button>
        </div>
        <div className="p-4">
            ...
            <div>
              {ownerProfile && (
                <div className="flex items-center gap-2 mb-2">
                  <Avatar name={ownerProfile.name || "Junction member"} src={ownerProfile.avatar_url} size={22} />
                  <span className="text-xs font-medium" style={{ color: T.sub }}>{ownerProfile.name || "Junction member"}</span>
                </div>
              )}
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", color: T.ink }} className="text-xl font-bold">AED {fmtAED(p.price)}</div>
              <div className="text-sm mt-0.5" style={{ color: T.ink }}>{p.title}</div>
              <div className="text-[11px] font-mono mt-0.5" style={{ color: T.sub }}>REF: JX-{String(p.id).replace(/-/g, "").slice(0, 7).toUpperCase()}</div>
            </div>
            <button onClick={() => onToggleLike(p.id)} className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ background: liked ? "#E0554C15" : T.panel }}>
              <Heart size={16} style={{ color: liked ? "#E0554C" : T.sub, fill: liked ? "#E0554C" : "none" }} />
            </button>
          </div>
          <div className="flex items-center gap-1 text-xs mt-2" style={{ color: T.sub }}><MapPin size={12} /> {p.area}, {p.emirate}</div>
          <div className="flex items-center gap-3 text-xs mt-2" style={{ color: T.sub }}>
            {p.beds != null && <span className="flex items-center gap-1"><BedDouble size={13} /> {p.beds}</span>}
            {p.baths != null && <span className="flex items-center gap-1"><Bath size={13} /> {p.baths}</span>}
            {p.sqft != null && <span className="flex items-center gap-1"><Maximize size={13} /> {p.sqft.toLocaleString()} sqft</span>}
          </div>
          {p.description && <p className="text-sm mt-3 leading-relaxed" style={{ color: T.ink }}>{p.description}</p>}

          {isOwner ? (
            <div className="flex gap-2 mt-4">
              <button onClick={() => { onEdit?.(p); onClose(); }} className="flex-1 text-sm font-bold py-2.5 rounded-xl" style={{ background: T.panel, color: T.navy }}>Edit</button>
              <button onClick={handleDelete} disabled={deleting} className="flex-1 text-sm font-bold py-2.5 rounded-xl" style={{ background: "#E0554C15", color: "#E0554C", opacity: deleting ? 0.6 : 1 }}>
                {deleting ? "Deleting…" : "Delete"}
              </button>
            </div>
          ) : (
            <button onClick={() => onChat?.(p)} className="w-full text-sm font-bold py-2.5 rounded-xl mt-4" style={{ background: T.ink, color: "#fff" }}>Contact lister</button>
          )}
  ...
  useEffect(() => {
    ...
    fetch("/api/properties?action=likes", { credentials: "include" })
      ...
      .then((data) => {
        ...
        const map = {};
        for (const id of data.likedIds) map[id] = true;
        setLiked(map);
      })
      .catch(() => {});
  }, [currentUserId]);
...
function PostPropertyModal({ onClose, statuses, onPublish }) {
  ...
  const handleVideoFile = async (file) => {
    ...
    try {
      ...
      const putRes = await fetch(urlData.signedUrl, { method: "PUT", body: file, headers: { "Content-Type": file.type || "video/mp4" } });
      if (!putRes.ok) { setPhotoError("Video upload failed — try a smaller file."); return; }
